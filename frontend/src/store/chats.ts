...
export const useChatsStore = create<ChatsState>((set, get) => {
	...
	return {
		...
		createChat: async (
			...
			const chat = await api.chats.create(storyId, {
				mode,
				activeSpeakers,
				memoryAnchors,
				startingLocationId,
			...
			await sendMessageStream({
				...
				onStateUpdate: (update) => {
					...
					if (update.newLocationCreated) {
						const { selectedStoryId, reloadLocations } =
		...
		regenerate: async (params: RegenerateParams) => {
			...
			set((s) => ({
				turns: [
					...s.turns.filter((t) => t.id !== lastAsst.id),
		...
		editAndResend: async (
			...
			useDebugStore.getState().clear();
			try {
				await api.chats.editTurn(
					activeStoryId,
					activeChatId,
					turnId,
					text,
				);
				await api.chats.deleteAfterTurn(
					activeStoryId,
					activeChatId,
					turnId,
				);
		...
		editTurn: async (turnId: string, text: string) => {
			...
			if (!activeChatId || !activeStoryId) return;
			try {
				const updated = await api.chats.editTurn(
					activeStoryId,
					activeChatId,
					turnId,
					text,
				...
				set((s) => ({
					turns: s.turns.map((t) => (t.id === turnId ? updated : t)),
				}));
			} catch (err) {
				set({
					error:
