...
function buildGraph(snapshot: ContextSnapshot): {
	nodes: GraphNode[];
	edges: GraphEdge[];
} {
	...
	const speaker = snapshot.characters.find(
		(c) => c.id === snapshot.activeSpeakerId,
	);
	if (speaker) {
		nodes.push(
			...placeRow(
	...
	nodes.push(
		...placeRow(
			snapshot.accessibleMemories.map((m) => ({
				id: m.id,
	...
	if (speakerNode) {
		...
		if (snapshot.currentLocationId) {
			const locNode = nodes.find(
				(n) => n.id === snapshot.currentLocationId,
			);
			if (locNode) {
				edges.push({
					x1: speakerNode.cx,
...
function buildCharFullLabel(c: ContextSnapshot['characters'][0]): string {
	...
	const personalityAdded = c.effectivePersonality.filter(
		(t) => !c.basePersonality.includes(t),
	);
	const personalityRemoved = c.basePersonality.filter(
		(t) => !c.effectivePersonality.includes(t),
	);
	if (personalityAdded.length > 0)
		parts.push(`+${personalityAdded.join(', ')}`);
	if (personalityRemoved.length > 0)
		parts.push(`−${personalityRemoved.join(', ')}`);
