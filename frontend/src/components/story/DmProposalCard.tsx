  ...
  return (
    <div class="bg-bg-tertiary border border-accent rounded p-3.5 flex flex-col gap-1.5">
      <div class="flex items-center gap-2">
        <span class={`text-[10px] font-bold tracking-[0.08em] uppercase py-[2px] px-[7px] rounded-full shrink-0 ${BADGE_CLS[proposal.type]}`}>
        ...
        <span class="text-[13px] font-semibold text-text-primary overflow-hidden text-ellipsis whitespace-nowrap">
          {typeof proposal.entityData.name === 'string' ? proposal.entityData.name : TYPE_LABEL[proposal.type]}
        </span>
      </div>
      {preview.slice(1).length > 0 && (
        <div class="flex flex-col gap-0.5">
          {preview.slice(1).map((line, i) => (
            <div key={i} class="text-xs text-text-secondary overflow-hidden text-ellipsis whitespace-nowrap">{line}</div>
          ))}
        </div>
      )}
      {proposal.rationale && (
        <div class="text-[11px] text-text-muted italic leading-[1.4]">{proposal.rationale}</div>
      )}
