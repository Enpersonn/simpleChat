import { ChatCreateSchema, ChatEntityStateSchema, ChatModeSchema } from "@simplechat/types";
...
export async function chatCRUDRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.patch<{ Params: { storyId: string; chatId: string } }>(
    ...
    async (req, reply) => {
      ...
      const parsedMode = body.mode !== undefined ? ChatModeSchema.safeParse(body.mode) : null;
  ...
  app.delete<{ Params: { storyId: string; chatId: string } }>(
    ...
    async (req, reply) => {
      ...
      await turn_store.replaceAll(allTurns.filter((t) => t.chatId !== chatId));
