...
export async function characterMemoriesRoutes(
  ...
  app.post<{ Params: { id: string; cid: string } }>(
    "/stories/:id/characters/:cid/memories",
    async (req, reply) => {
      const body = CharacterMemoryWithRelationCreateSchema.safeParse(req.body);
      if (!body.success)
        return reply.status(400).send({ error: body.error.flatten() });

      const { previousRelationId, branchLabel, ...memContent } = body.data;
      ...
      if (!previousRelationId) {
        ...
        if (heads.length > 1) {
          return reply.status(409).send({
            error:
              "Multiple memory branches exist. Specify previousRelationId to indicate which branch to extend.",
            heads: heads.map((h) => ({
              id: h.id,
    ...
    async (req, reply) => {
      ...
      if (branchLabel !== undefined || previousRelationId !== undefined) {
        ...
        if (rel) {
          ...
          if (branchLabel !== undefined) updates.branchLabel = branchLabel;
          if (previousRelationId !== undefined)
            updates.previousRelationId = previousRelationId;
          await character_memory_relations_store.update(rel.id, updates);
        }
      }
  ...
  app.delete<{ Params: { id: string; cid: string; mid: string } }>(
    ...
    async (req, reply) => {
      ...
      const rel = relations[0];
      if (!rel) return reply.status(404).send({ error: "Memory not found" });
