...
export function normaliseMemoryItem(m: Record<string, unknown>) {
	...
	const effects = (rawEffects as unknown[])
		...
		.map((e) => ({
			...
			entityType:
				typeof e.entityType === 'string' ? e.entityType : 'character',
	...
	return {
		...
		storyOrder:
			typeof m.storyOrder === 'number' ? Math.trunc(m.storyOrder) : 0,
...
function normaliseStoryRules(raw: unknown) {
	if (Array.isArray(raw))
		return {
			worldRules: raw.filter((x): x is string => typeof x === 'string'),
			storyRules: [],
			characterRules: [],
		};
	if (typeof raw === 'object' && raw !== null) {
		const r = raw as Record<string, unknown>;
		return {
			worldRules: Array.isArray(r.worldRules)
				? r.worldRules.filter((x): x is string => typeof x === 'string')
				: [],
			storyRules: Array.isArray(r.storyRules)
				? r.storyRules.filter((x): x is string => typeof x === 'string')
				: [],
			characterRules: Array.isArray(r.characterRules)
				? r.characterRules.filter(
						(x): x is string => typeof x === 'string',
					)
				: [],
		};
