...
function schemaToExample(schema: z.ZodTypeAny): unknown {
  ...
  if (schema instanceof z.ZodEnum) return (schema.options as unknown[])[0] ?? "example value";
  if (schema instanceof z.ZodArray) return [schemaToExample(schema.element), schemaToExample(schema.element)];
  if (schema instanceof z.ZodRecord) return {};
  if (schema instanceof z.ZodOptional) return schemaToExample(schema.unwrap());
  if (schema instanceof z.ZodNullable) return schemaToExample(schema.unwrap());
...
export const createPromptRunner = <T extends z.ZodTypeAny>(config: {
  ...
  const exampleJson = JSON.stringify(schemaToExample(config.outputSchema), null, 2);
