...
const RESPONSE_LENGTH_MAP: Record<string, string> = {
	...
	'paragraph+':
		'Write an extended, immersive passage of 8 or more paragraphs.',
};
...
function resolveTokens(text: string, characters: Character[]): string {
	...
	return text
		...
		.replace(
			/\{\{char_2\}\}/g,
			nonPersonas[1]?.name ?? 'the second character',
		)
		...
		if (speaker) {
			...
			if (hasState) {
				...
				if (speaker.public.personality.length > 0)
					parts.push(
						`Observable traits: ${speaker.public.personality.join(', ')}.`,
					);
				if (speaker.public.speechStyle)
					parts.push(`Speech style: ${speaker.public.speechStyle}.`);
				if (speaker.private.trueMotives)
					parts.push(
						`Private motivations (never reveal directly): ${speaker.private.trueMotives}.`,
					);
				if (speaker.private.hiddenEmotionalState)
					parts.push(
						`Hidden emotional state: ${speaker.private.hiddenEmotionalState}.`,
					);
				if (speaker.private.fears.length > 0)
					parts.push(
						`Hidden fears: ${speaker.private.fears.join(', ')}.`,
					);
			}

			if (userPersonas.length > 0) {
				const personaNames = userPersonas
					.map((p) => p.name)
					.join(' and ');
				parts.push(
...
function buildPersonasBlock(userPersonas: Character[]): string {
	...
	const descriptions = userPersonas
		.map((p) => {
			...
			if (p.public.speechStyle)
				parts.push(`speaks: ${p.public.speechStyle}`);
			return parts.join(' — ');
		})
		.join('; ');
...
export function buildCharacterLocationFeelingBlock(
	...
	const rel = speaker.locationRelationships?.find(
		...
		const comfortLabel =
			rel.comfort >= 7
				...
				: rel.comfort <= 3
					...
					: 'neutral';
		const tensionLabel =
			rel.tension >= 7 ? 'very tense' : rel.tension >= 4 ? 'tense' : null;
		const stateDesc = [comfortLabel, tensionLabel]
			.filter(Boolean)
			.join(', ');
...
export function assembleContext(opts: AssembleOptions): OllamaMessage[] {
	...
	if (currentLocation) {
		systemParts.push(
			buildLocationBlock(currentLocation, locationOverrides),
		);
		if (speaker) {
			const feelingBlock = buildCharacterLocationFeelingBlock(
				speaker,
				currentLocation.id,
			);
